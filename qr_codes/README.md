...
모든 QR의 목적지는 공통 목적지 `(4.00, 4.00) m`다. 화물 조건만 로봇 배정에 사용한다.

| 파일 | 화물 | 배정 로봇 | 목적지 좌표 |
| --- | --- | --- | --- |
| `01_SMALL_DELIVERY.png` | 소형 상자 2 kg | 1대 | (4.00, 4.00) m |
| `02_LONG_DELIVERY.png` | 장형 상자 8 kg | 2대 | (4.00, 4.00) m |

QR 내부 데이터 형식은 다음과 같다. `dest_x`, `dest_y`를 바꾸면 새 목적지를 만들 수 있다.

```json
{"command":"DELIVERY","cargo_type":"LONG_BOX","weight_kg":8.0,"dest_x":4.00,"dest_y":4.00}
```
