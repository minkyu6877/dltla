...
Raspberry Pi 한 대가 QR을 인식하고 Wi-Fi/UDP로 두 대의 ESP32 메카넘 로봇을 제어하는 프로젝트입니다. 저장소에는 실제 구동 코드, UWB 캘리브레이션 도구, 시뮬레이션 코드가 함께 정리되어 있습니다. UWB를 사용하지 않는 모드에서는 엔코더 이동거리와 MPU6050 방향을 결합해 짧은 임무 동안 지도 좌표를 추정합니다.
